...
//  by following the suffix_node link to a new node.  sNodes
...
// is already present in the sNodes[] array, and its
// suffix node is set to -1 by the default sNode() ctor,
...
int Edge::SplitEdge( Suffix &s )
{
    ...
    Edge *new_edge =
      new Edge( first_char_index,
                ...
                s.origin_node );
    new_edge->Insert();
    sNodes[ new_edge->end_node ].suffix_node = s.origin_node;
...
void dump_edges( int current_n )
{
    ...
    for ( int j = 0 ; j < HASH_TABLE_SIZE ; j++ ) {
        ...
        cout << setw( 5 ) << s->start_node << " "
             << setw( 5 ) << s->end_node << " "
             << setw( 3 ) << sNodes[ s->end_node ].suffix_node << " "
...
//
        ...
        if ( last_parent_node > 0 )
            sNodes[ last_parent_node ].suffix_node = parent_node;
...
//
        if ( active.origin_node == 0 )
            ...
            active.origin_node = sNodes[ active.origin_node ].suffix_node;
        active.Canonize();
    }
    if ( last_parent_node > 0 )
        sNodes[ last_parent_node ].suffix_node = parent_node;
...
void validate()
{
    ...
    cout << "Branch count : "
         << branch_count
         << ( branch_count == (sNode::Count - 1) ? " OK" : " Error!" )
         << endl;
}
...
void print_seq_runtime(char16_t* string){
	//test
