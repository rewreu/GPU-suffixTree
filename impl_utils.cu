...
__device__ void combineNode(Node** address, Node* node2, char16_t* text){
	Node* node1 = *address;
	int i = node1->start;
	int j = node2->start;
	int i_end = node1->end;
...
__device__ void addNode(Node** address, Node* child, char16_t* text){
...
__device__ int nodeToString(char16_t* buf, Node* node, char16_t* text){
...
__device__ void printTreeRe(Node* root, char16_t* text, int indent){
	for(int i = 0; i < NUM_CHILDREN; i++){
...
// pre-order traversal
__host__ __device__ void serialize(Node* root, char16_t* text, char16_t* output, int* counter){
    // base case
    if(root == NULL) return;
...
__global__ void serialization(Node* root, char16_t* text, char16_t* output){
	int counter = 0;
...
int getSerialSuffixTree(Node* d_root, char16_t* d_text, char16_t** output){
	int size = 0;
	int* d_size = NULL;
	char16_t* d_output = NULL;
